...
# =====================
DB_FOLDER = "db"  # base folder
QUESTIONS_FOLDER = os.path.join(DB_FOLDER, "Questions")  # folder containing multiple question files
EMP_STD_FILE = os.path.join(DB_FOLDER, "Result 2.xlsx")  # Sheets: "Emloyees Data", "Standard", "Result"
INFO_FILE = os.path.join(DB_FOLDER, "info.xlsx")         # One sheet per Standard short name
...
def get_info_for_standard(standards_df, selected_standard):
    if standards_df.empty or selected_standard == "":
        return 0, 0, "00", "00", "00"
    try:
        short_name = standards_df.loc[standards_df["Standard"].str.upper() == str(selected_standard).strip().upper(), "ShortName"].values[0]
    except Exception:
...
def append_result(emp_id, emp_name, total, right, wrong, criteria_pct, status, test_type):
    try:
        ...
        try:
            ...
            df_old = pd.DataFrame(columns=["ID","Name","Total","Right","Wrong","%","Criteria%","Status","Type","DateTime"])
        cols = df_old.columns.tolist() if len(df_old.columns) else ["ID","Name","Total","Right","Wrong","%","Criteria%","Status","Type","DateTime"]
...
if "quiz" not in st.session_state:
    with st.form("login_form", clear_on_submit=False):
        ...
        with c2:
            st.metric("Passing Criteria (%)", criteria)
        with c3:
            st.metric("Timer (HH:MM:SS)", f"{h}:{m}:{s}")
        ...
        if submitted:
            ...
            elif selected_standard not in options:
                st.error("Select a valid Standard.")
            else:
    ...
    with t1:
        st.caption(f"ID: {qstate['emp_id']}  •  Name: {qstate['emp_name']}")
    with t2:
        st.caption(f"Standard: {qstate['standard']}")
    with t3:
        st.caption(f"Question {qstate['q_index']+1} of {qstate['total']}")

    st.info(f"Time Left: {rem_h:02d}:{rem_m:02d}:{rem_s:02d}")
    if total_secs > 0 and remaining <= 0:
        st.warning("Time is up! Submitting your test...")
        qstate["wrong"] += max(0, qstate["total"] - qstate["right"] - qstate["wrong"])
        st.session_state.quiz = qstate

    if qstate["q_index"] < qstate["total"] and (total_secs == 0 or remaining > 0):
        row = qstate["rows"].iloc[qstate["q_index"]]
        qno, question, A, B, C, D, correct = row["Qno"], row["Question"], row["A"], row["B"], row["C"], row["D"], row["Answer"]
        st.subheader(f"Q{qstate['q_index']+1}. {question}")
        choice = st.radio("Choose your answer:", [A, B, C, D], index=None, key=f"q_{qstate['q_index']}")

        col1, col2 = st.columns([1,1])
        with col1:
            if st.button("Next", use_container_width=True):
                if choice is None:
                    st.warning("Please select an option before moving on.")
                else:
                    mapping = {"A": A, "B": B, "C": C, "D": D}
                    correct_text = mapping.get(str(correct).strip(), str(correct).strip())
                    if str(choice).strip() == str(correct_text).strip():
                        qstate["right"] += 1
                    else:
                        qstate["wrong"] += 1
                    qstate["answers"].append((qno, choice, correct_text))
                    qstate["q_index"] += 1
                    st.session_state.quiz = qstate
                    st.rerun()
        with col2:
    ...
    if qstate["q_index"] >= qstate["total"] or (total_secs > 0 and remaining <= 0):
        right = qstate["right"]
        wrong = qstate["wrong"]
        total_q = qstate["total"]
        ...
        # if qstate["answers"]:
        #     df_ans = pd.DataFrame(qstate["answers"], columns=["Qno","Your Answer","Correct Answer"])
        #     st.dataframe(df_ans, use_container_width=True)

        test_type = qstate["standard"]
        ok, msg = append_result(qstate["emp_id"], qstate["emp_name"], total_q, right, wrong, criteria, status, test_type)
        if ok:
            st.info("Result saved to 'Result' sheet in 'Result 2.xlsx'.")
        else:
            st.warning("Could not write result to Excel. You can download your results below.")
            out = pd.DataFrame([{
                "ID": qstate["emp_id"],
            ...
            st.download_button("Download Result CSV", out.to_csv(index=False).encode("utf-8"), file_name="result.csv", mime="text/csv")
