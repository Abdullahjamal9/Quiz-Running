...
# ✅ Modified append_result (no Excel writing)
...
if "quiz" not in st.session_state:
    ...
    if qstate["q_index"] >= qstate["total"] or (total_secs > 0 and remaining <= 0):
        ...
        ok, result_data = append_result(qstate["emp_id"], qstate["emp_name"], total_q, right, wrong, criteria, status, qstate["standard"])
        if ok:
            out = pd.DataFrame([result_data])
            st.download_button(
                "📥 Download Your Result (CSV)",
                out.to_csv(index=False).encode("utf-8"),
                file_name=f"result_{qstate['emp_id']}.csv",
                mime="text/csv"
            )
        else:
            st.warning(f"Error generating result: {result_data}")
